class Component {
    ...
    /*
    //Get an array of all the components used by a template
    static find(transform) {
        
        let all = [];
        
        //Check for components in this array
        if(Array.isArray(transform)) {
            
            //Traverse the array
            for(let _transform of transform) 
                all = all.concat(all, this.find(_transform));
        }
        ...
        if(typeof(transform) === "object") {
            ...
            //Check for the component at this level
            if(typeof(component) === "string") all[component] = 1;
            
            //Check for sub components in the html for this object
            if(typeof(transform.html) === "object") all = all.concat(all, this.find(transform.html));
        }
